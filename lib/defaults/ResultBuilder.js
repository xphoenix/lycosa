...
//	        "comment": ""
		...
		if (t.hasErrors()) {
			...
			// Insert status
			entry.response.status = t.fetchResult.status;
			entry.response.statusText = t.fetchResult.statusText;

			// Merge headers. Please note that  Fetcher might adds
			// some new request headers, so we need to copy it
			// again
			insertHeaders(entry.request, t.fetchResult.request);
			insertHeaders(entry.response, t.fetchResult.response);
			...
			entry.response.bodySize = t.fetchResult.receivedSize;
			entry.response.content.size = t.fetchResult.logicalSize;
			entry.response.content.compression = t.fetchResult.receivedSize;
			entry.response.content.mimeType = t.fetchResult.response['content-type'];
		}
