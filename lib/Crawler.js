		...
		if (!this._requests.hasOwnProperty(id)) {
			...
			this._requests[id] = {
					promise: this._prepare(trace, ip).then(function(){
						...
						// Some Crawler internal error happens. We don't want to
						// loose all available traces for the request as it is error
						// context information, however AFTER Error arise it must be processed
						// and there is no guarantee that V8 will continue to work normally.
						//
						// Usually we need to shutdown process, but we need to return traces...
						// TODO: FIX THAT!
						var result = self._requests[id].traces;
						result[result.length-1].addGenericError(error);
...
Crawler.prototype._prepare = function (trace, ip, cached, session) {
	...
	return when.join(ip, cached, session).then(function(results){
		...
		//
		// TODO: move cookies resolving to the next stage
		...
		if (trace.hasErrors()) {
			...
			// be full & complete. That error should be treats as a crawler internal
			// error as indicates some serious mistake in system invarians stability
			throw new Error("That should never happens");
		}
	}).ensure(function(){
...
Crawler.prototype._fetch = function(trace) {
	...
	return this._behave('fetchPageContent', trace).then(function(fetchResult){
		...
		cookies.forEach(function (c){
			trace._session.cookiejar.setCookieSync(c, trace.url, {ignoreError: true});
		});
