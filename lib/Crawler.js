 ...
 * has special format:
 *
 * 	function behavior(crawler, sequence, argument, callback);
 *
 * Where
 * +	**crawler**  - instance of crawler that request operation;
 * +	**sequence** - string id of page fetching sequence initiated request
 * +	**argumnet** - operation dependent argument, for example hostname;
 ...
 *  c.setup('resolveHost', function(crawler, sequence, host, resolveCallback){
 *
 *  });
 *
 * @method setup
...
Crawler.prototype.setup = function(name, impl) {
	...
	this._behaviors[name] = nodefn.lift(impl, this);
};
...
Crawler.prototype._behave = function(name, seq) {
	...
	if (op === false) {
		...
		return op.apply(null, Array.prototype.slice.call(arguments, 1)).then(function(result){
			return {
				value: result,
				time: Date.now() - start
			};
