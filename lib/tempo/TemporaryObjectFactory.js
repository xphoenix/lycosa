...
TemporaryObjectFactory.prototype.get = function (timeout, key) {
	...
	if (!this.has(key)) {
		...
		promise = this.buildAction.apply(null, Array.prototype.slice.call(self_args, 2));
...
TemporaryObjectFactory.prototype.__install = function (key, promise, timeout) {
	...
	var self = this;
	this._entries[key] = promise = promise.otherwise(function(error) {
		// It could be that timer is already setup, so in that case - clear it
		if (promise[_kTimer]) {
			clearTimeout(promise[_kTimer]);
		}
 ...
 * @private
 */
TemporaryObjectFactory.prototype.__delete = function(key, args) {
	...
	value.then(function(obj){
		try {
			return value[_kDeferer].resolve(self.destroyAction.apply(null, args));
		} catch(err) {
